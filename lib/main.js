...
module.exports = {
  ...
  async runCalc(parser, parameters = "", version) {
    ...
    for (let pathSrc of pathSrcs) {
      ...
      new BufferedProcess({ command: parserPath, args: [pathSrc, ...parameters.split(/\s+/).filter(Boolean)] });
  ...
  runCalcCurrentNow(parser, version) {
    ...
    editor.backwardsScanInBufferRange(/(?:prog|sys|apply|chapter) /gi, range, (object) => {
      ...
      new BufferedProcess({ command: `${sofPath}\\${parser}.exe`, args: [pathSrc, `-run:${line + 1}`, "-e"] });
      object.stop();
      return;
    });
  },
  ...
  exportPLB2DOCX(version) {
    ...
    new BufferedProcess({ command: `${sofPath}\\plbdocx.exe`, args: ["-f", pathSrc, "-o", pathDst] });
  },
