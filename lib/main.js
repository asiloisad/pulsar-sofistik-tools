...
export default {
  config: {
    envPath: {
      ...
      type: 'boolean',
      title: "Scroll to keyword in help documents [experimental]",
      description: "Open help on the page with the current key. In most cases you want use this, but not all PDF viewer in atom support it. The supported and recommended PDF viewer is [pdf-viewer](https://atom.io/packages/pdf-viewer). The option can be activated only for inner PDF viewers. **Only English**",
      order: 4,
      default: false,
    },
  },
  ...
  activate (_state) {
    ...
    this.subscriptions.add(atom.commands.add('.tree-view', {
      ...
      'SOFiSTiK-tools:clean-[S]-[ERG-PRT-LST-URS-SDB-PL-$??-#??-GRB-CDI-CDE-CDB-PLB-BAK-SDB-DOCX-CFG]':
        () => this.cleanSelectedFolders(14),
      'SOFiSTiK-tools:clean-[R]-[ERG-PRT-LST-URS-SDB-PL-$??-#??-GRB-CDI-CDE-CDB-PLB-BAK-SDB-DOCX-CFG]':
  ...
  deactivate () {
    ...
    if (this.helpListView) {this.helpListView.destroy()}
    if (this.versListView) {this.versListView.destroy()}
  },
  ...
  openHelpPDF() {
    if (!this.helpListView) {
      this.helpListView = new HelpListView();
      this.helpListView.getSofPath = this.getSofPath
    }
    this.helpListView.toggle()
  },
  ...
  changeVersion(){
    if (!this.versListView) {
      this.versListView = new VersListView();
    }
    this.versListView.toggle()
  },
  ...
  parseFilter(filter) {
    if        (filter===11) {
      return    "*@(.erg|.prt|.lst|.urs|.sdb|.pl|.$??|.#??|.grb)"
    } else if (filter===21) { return '**/'+this.parseFilter(11)

    } else if (filter===12) {
      return    "*@(.erg|.prt|.lst|.urs|.sdb|.pl|.$??|.#??|.grb|.cdi|.cde)"
    } else if (filter===22) { return '**/'+this.parseFilter(12)

    } else if (filter===13) {
      return    "*@(.erg|.prt|.lst|.urs|.sdb|.pl|.$??|.#??|.grb|.cdi|.cde|.cdb)"
    } else if (filter===23) { return '**/'+this.parseFilter(13)

    } else if (filter===14) {
      return    "*@(.erg|.prt|.lst|.urs|.sdb|.pl|.$??|.#??|.grb|.cdi|.cde|.cdb|.plb|.bak|.sdb|.docx|.cfg|_csm.dat|_csmlf.dat)"
    } else if (filter===24) { return '**/'+this.parseFilter(14)
  ...
  currentHelp(mode, version) {
    editor = atom.workspace.getActiveTextEditor()
    range = [Point.ZERO, editor.getCursorBufferPosition()]
