...
export default {
  ...
  childrenLookup() {
    ...
    let pathSrcs = [] ; let editorPath = editor.getPath() ; if (!editorPath) {return}
    let cmode = false
    editor.scan(/^@ +only-children/gm, (item) => {
      cmode = true ; item.stop()
    })
    if (!cmode) { pathSrcs.push(editorPath) }
    let pathChild
    editor.scan(/^@ child:(.+)$/gm, (item) => {
      pathChild = path.join(path.dirname(editorPath), item.match[1].trim())
      pathSrcs.push(pathChild)
    })
    return pathSrcs
  },
  ...
  async runCalc(parser, parameters='', version) {
    const editor = atom.workspace.getActivePaneItem()
    let pathSrcs = this.childrenLookup()
    let parserPath = `${this.getSofPath(version)}\\${parser}.exe`
    editor.save()
  ...
  openViewer(parameters='', version) {
    ...
    let pathSrc = editor.getPath() ; if (!pathSrc) {return}
    pathSrc = pathSrc.replace('.dat', '.cdb');
    exec(`"${this.getSofPath(version)}\\fea_viewer.exe" ${parameters} "${pathSrc}"`)
  },
  ...
  openDBInfoNG(version) {
    ...
    let pathSrc = editor.getPath() ; if (!pathSrc) {return}
    pathSrc = pathSrc.replace('.dat', '.cdb');
    exec(`"${this.getSofPath(version)}\\dbinfo_ng.exe" "${pathSrc}"`)
  },
  ...
  openSOFiPLUS(version) {
    ...
    let pathDWG = false
    if (editor) {
      if (pathSrc = editor.getPath()) {
        pathDWG = pathSrc.replace('.dat', '.dwg');
      }
    }
    if (pathDWG===false) {
      exec(`"${this.getSofPath(version)}\\sofiplus_launcher.exe"`)
    } else {
      exec(`"${this.getSofPath(version)}\\sofiplus_launcher.exe" "${pathDWG}"`)
    }
  },
  ...
  cleanCustomFilters() {
    let ltv = new TextView('', false, false, 'Enter the glob pattern to find the files to be deleted')
  ...
  currentHelp(mode, version) {
    ...
    let prog, pro2, lang, postLang, sofPath, flag
