...
export default {
  ...
  runSOFiSTiK(prog, ext, filePath, props, prop2) {
    ...
    if (prop2 && prop2.existsCDB && !fs.existsSync(this.changeExtension(filePath, '.cdb'))) {
      return atom.notifications.addWarning(`Database doesn't exists "${filePath.replace(/\\/g, '\\\\')}"`)
    }
    let exists = fs.existsSync(filePath)
    if (prop2 && prop2.existsSkip) {
      args.push(filePath) // always add, e.g. WinGRAF
    } else if (prop2 && prop2.existsOnly) {
      if (exists) { args.push(filePath) } // only if exists, e.g. SOFiPLUS
    } else if (!exists) {
