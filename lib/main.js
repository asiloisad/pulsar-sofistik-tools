...
export default {
  ...
  singleViewer(object, range, filePath, prog) {
    ...
    if (!atom.packages.isPackageActive('pdf-viewer')) {
      ...
      editor.backwardsScanInBufferRange(regex, nange, (finder) => {
        let dest = finder.match[1].toUpperCase()
        if (this.viewer) {
          if (this.viewer.getPath()===filePath) {
            this.viewer.hash = `#nameddest=${dest}&SOFiSTiK`
            this.viewer.scrollToDestination({ dest:dest, destHash:`#${dest}` })
          } else {
            this.viewer.setFile(filePath, `#nameddest=${dest}&SOFiSTiK`)
            this.viewer.updateTitle()
            this.viewer.reload()
          }
        } else {
          atom.workspace.open(`${filePath}#nameddest=${dest}&SOFiSTiK`, { split:"right", activatePane:false }).then( (resolve, _) => {
            this.viewer = resolve
  ...
  multiViewer(object, range, filePath, prog) {
    ...
    if (atom.packages.isPackageActive('pdf-viewer')) {
      ...
      editor.backwardsScanInBufferRange(regex, nange, (finder) => {
        let dest = finder.match[1].toUpperCase()
        atom.workspace.open(`${filePath}#nameddest=${dest}&${this.makeID(9)}`, { split:"right", activatePane:false })
        this.findQ = true
        finder.stop()
      })
    }
