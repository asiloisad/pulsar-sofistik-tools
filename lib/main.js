...
export default {
  ...
  currentHelp(mode, version) {
    ...
    editor.backwardsScanInBufferRange(/[\+\-\$]prog +(\w+)/, range, (object)=>{
      ...
      if (fs.existsSync(filePath = path.join(this.getSofPath(version), `${prog}${postLang}.pdf`))) {
        flag = true
      } else if  (fs.existsSync(filePath = path.join(this.getSofPath(version), `${prog}.pdf`))) {
        flag = true
      } else {
        flag = false
      }
      if (flag) {
        ...
        atom.notifications.addWarning(`Cannot find the manual for program "${prog.toUpperCase()}"`)
      }
      object.stop()
    })
  },
