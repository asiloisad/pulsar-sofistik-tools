...
export default {
  ...
  config: {
    ...
    findKey: {
      ...
      description: "Open help on the page with the current key. In most cases you want use this, but not all PDF viewer in atom support it. The supported and recommended PDF viewer is [pdf-viewer](https://atom.io/packages/pdf-viewer). The option has effect only for inner PDF viewers. **Only English**",
      type: 'boolean',
      default: false,
    },
  },
  ...
  singleViewer(object, range, filePath, prog) {
      ...
      editor.backwardsScanInBufferRange(regex, nange, (finder)=>{
        if (this.viewer) {
          if (this.viewer.getPath()===filePath) {
            ...
            this.viewer.setFile(filePath, '#'+finder.match[1].toUpperCase())
            this.viewer.updateTitle()
            this.viewer.reload()
          }
        } else {
          atom.workspace.open(`${filePath}#nameddest=${finder.match[1].toUpperCase()}`, {split:"right", activatePane:false}).then( (resolve, _) => {
            this.viewer = resolve
    ...
    if (!this.findQ) {
      if (this.viewer) {
        if (this.viewer.getPath()!==filePath) {
          this.viewer.setFile(filePath, false)
          this.viewer.updateTitle()
          this.viewer.reload()
        }
      } else {
        atom.workspace.open(filePath, {split:"right", activatePane:false}).then( (resolve, _) => {
          this.viewer = resolve
          this.viewer.onDidDispose(() => {this.viewer = null})
        })
      }
  ...
  multiViewer(object, range, filePath, prog) {
    this.findQ = false
    if (atom.config.get('sofistik-tools.findKey') || atom.packages.isPackageActive('pdf-viewer')) {
      regex = new RegExp('(?:^[ \\t]*|; *)('+keywords[prog].join('|')+')\\b', 'i')
      nange = [object.range.start, range[1]]
      editor.backwardsScanInBufferRange(regex, nange, (finder)=>{
