...
export default {
  ...
  activate (_state) {
    ...
    this.subscriptions.add(atom.commands.add('.tree-view', {
      'SOFiSTiK-tools:clean-[S]-[ERG-PRT-LST-PL-$??-#??-GRB]':
        () => this.cleanSelectedFolders(11),
      'SOFiSTiK-tools:clean-[R]-[ERG-PRT-LST-PL-$??-#??-GRB]':
        () => this.cleanSelectedFolders(21),
      'SOFiSTiK-tools:clean-[S]-[ERG-PRT-LST-PL-$??-#??-GRB-CDI-CDE]':
        () => this.cleanSelectedFolders(12),
      'SOFiSTiK-tools:clean-[R]-[ERG-PRT-LST-PL-$??-#??-GRB-CDI-CDE]':
        () => this.cleanSelectedFolders(22),
      'SOFiSTiK-tools:clean-[S]-[ERG-PRT-LST-PL-$??-#??-GRB-CDI-CDE-CDB]':
        () => this.cleanSelectedFolders(13),
      'SOFiSTiK-tools:clean-[R]-[ERG-PRT-LST-PL-$??-#??-GRB-CDI-CDE-CDB]':
  ...
  cleanByPaths(dirPaths, filter) {
    // predefined filters
    if        (filter===11) {
      filter =    "*@(.erg|.prt|.lst|.pl|.$??|.#??|.grb)"
    } else if (filter===12) {
      filter =    "*@(.erg|.prt|.lst|.pl|.$??|.#??|.grb|.cdi|.cde)"
    } else if (filter===13) {
      filter =    "*@(.erg|.prt|.lst|.pl|.$??|.#??|.grb|.cdi|.cde|.cdb)"
    } else if (filter===21) {
      filter = "**/*@(.erg|.prt|.lst|.pl|.$??|.#??|.grb)"
    } else if (filter===22) {
      filter = "**/*@(.erg|.prt|.lst|.pl|.$??|.#??|.grb|.cdi|.cde)"
