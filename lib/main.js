...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      atom.commands.add('atom-text-editor[data-grammar="source sofistik"]', {
        ...
        "sofistik-tools:current-help-separately": () => this.currentHelp(2),
        ...
        "sofistik-tools:progam-current-toggle": () => this.changeProg(),
        "sofistik-tools:progam-all-toggle": () => this.changeProgs("all"),
        "sofistik-tools:progam-all-ON": () => this.changeProgs("all", "ON"),
        "sofistik-tools:progam-all-OFF": () => this.changeProgs("all", "OFF"),
        "sofistik-tools:progam-above-toggle": () => this.changeProgs("above"),
        "sofistik-tools:progam-above-ON": () => this.changeProgs("above", "ON"),
        "sofistik-tools:progam-above-OFF": () =>
          this.changeProgs("above", "OFF"),
        "sofistik-tools:progam-below-toggle": () => this.changeProgs("below"),
        "sofistik-tools:progam-below-ON": () => this.changeProgs("below", "ON"),
        ...
        "sofistik-tools:open-viewer-2025": () =>
          this.editorOpenViewer({ version: "2025" }),
  ...
  loader() {
    ...
    try {
      this.keywords = JSON.parse(fs.readFileSync(keyPath));
    } catch (e) {
      return;
    }
  },

  getVersion(version, filePath) {
    if (!version) {
      const editor = atom.workspace.getActiveTextEditor();
      if (editor && editor.getGrammar().scopeName === "source.sofistik") {
        editor.backwardsScanInBufferRange(
          /^@ *SOFiSTiK *(\d{4})(-\d\d?)? *$/,
          [Point.ZERO, editor.getCursorBufferPosition()],
          (item) => {
    ...
    if (!version) {
      // Try to get path from filePath argument, or from active editor
  ...
  async runCalc(parser, parameters = "", version) {
    ...
    let sofPath = this.getSofPath(version);
    if (!sofPath) {
      return;
    }
  ...
  openViewer(filePath, props) {
    const version = parseInt(this.getVersion(props ? props.version : null));
    if (version >= 2024) {
  ...
  openTeddy(filePath, props) {
    console.log("openTeddy", filePath, props);
  ...
  consumeOpenExternalService(service) {
    const handler = {
      ...
      canHandle: (filePath) => {
        ...
        return [".sofistik", ".cdb", ".gra", ".results", ".plb"].includes(ext);
      },
  ...
  getViewer(filePath, dest, tag) {
    ...
    if (viewer) {
      if (viewer.getPath() === filePath) {
        ...
        viewer.setFile(filePath, destCmd);
        viewer.reload();
      }
    } else {
      atom.workspace.open(`${filePath}${destCmd}`, {
        split: "right",
        activatePane: false,
      });
    }
  ...
  inViewer(object, range, filePath, prog, tag) {
    ...
    if (this.keywords) {
      let regex = new RegExp(
        "(?:^[ \\t]*|; *)(" +
          Object.keys(this.keywords[prog]).join("|") +
          ")\\b",
        "i"
      );
    ...
    if (!this.findQ) {
      this.getViewer(filePath, false, tag);
    }
  },
