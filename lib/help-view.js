...
export default class HelpView {
  ...
  async show () {
    ...
    lang = atom.config.get('sofistik-atom.lang')
    if (lang=='english') {
      this.postLang = '_1'
    } else if (lang=='germany') {
      this.postLang = '_0'
    } else {
      atom.notifications.addWarning('Language "'+lang+'" is not supported, English is used instead')
      this.postLang = '_1'
    }

    this.path11 = this.getSofPath()[1]
