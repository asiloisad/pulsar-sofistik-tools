...
export default class HelpList {
  ...
  constructor (S) {
    this.S = S ; this.items = null ; this.query = ''
    ...
    this.slv = new SelectListView({
      ...
      filter: (items, query) => {
        ...
        if (colon !== -1) {
          this.query = query.slice(0, colon) ; this.qdest = query.substring(colon+1)
        } else {
          this.query = query ; this.qdest = false
        }
        let scoredItems = []
        for (let item of items) {
          item.score = zadeh.score(item.displayName, this.query)
  ...
  didConfirmSelection(item, mode) {
    ...
    if (mode==='open-in') {
      if (this.qdest) { filePath += `#nameddest=${this.qdest.toUpperCase().trim().replace(/ /g, '')}` }
